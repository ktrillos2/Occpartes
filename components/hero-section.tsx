...
export function HeroSection({ data }: { data?: any }) {
  ...
  const headline = data?.headline || { line1: "Repuestos y Servicio", line2: "Especializado", line3: "para Equipos Volvo" }
  const description = data?.description || "Más de 15 años de experiencia en maquinaria de construcción, camiones, buses y equipos marinos. Cubrimos el 100% del suministro de partes Volvo."
  const badgeText = data?.badgeText || "Distribuidores Autorizados SLP en Colombia"
  const serviceHighlightsData = data?.serviceHighlights || serviceHighlights
  ...
  // We need to import urlFor. Since we can't easily add imports with replace_file_content without context, 
  // I will assume I can add it at the top or I will use a simple check.
  // Actually, I should use `multi_replace` to add the import. 
  // For this step, I'll just use the logic with a placeholder and then add the import.
  ...
  return (
    <section id="inicio" className="relative min-h-screen overflow-hidden bg-[#0A1628]">
      ...
      <AnimatePresence mode="wait">
        ...
        >
          {/* We need the image builder here. I'll hack it for now to use fallback if no builder */}
          {/* Note: I will need to update the imports in a separate call or use multi_replace */}
          <SanityImage
            source={slides[currentSlide]?.image}
            fallback={typeof slides[currentSlide]?.image === 'string' ? slides[currentSlide]?.image : "/placeholder.svg"}
      ...
      <div className="relative z-10 container mx-auto px-4 min-h-screen flex flex-col justify-center">
        <div className="grid lg:grid-cols-12 gap-8 items-center py-20">
          ...
          <div className="lg:col-span-7 space-y-8">
            ...
            >
              <h1 className="text-4xl md:text-5xl lg:text-6xl xl:text-7xl font-extrabold text-white leading-[1.1]">
                ...
                <span className="block text-[#F7A600]">{headline.line2}</span>
                <span className="block text-white/90">{headline.line3}</span>
              </h1>
            </motion.div>
            ...
            >
              ...
              >
                <a href={data?.primaryCta?.url || "https://wa.me/573218644235"} target="_blank" rel="noopener noreferrer">
                  ...
                  {data?.primaryCta?.text || "Solicitar Cotización"}
              ...
              >
                <a href={data?.secondaryCta?.url || "#servicios"}>{data?.secondaryCta?.text || "Explorar Servicios"}</a>
              </Button>
            </motion.div>
          </div>
          ...
          <div className="lg:col-span-5 hidden lg:block">
            ...
            >
              {serviceHighlightsData.map((service: any, index: number) => (
                <motion.div
                  key={index}
        ...
        <div className="absolute bottom-8 left-4 right-4 lg:left-auto lg:right-8 flex items-center gap-6">
          ...
          >
            <p className="text-white/50 text-sm uppercase tracking-wider">{slides[currentSlide].subtitle}</p>
            <p className="text-white font-bold text-xl">{slides[currentSlide].title}</p>
          </motion.div>
...
// Helper component for Image to handle string urls and Sanity image records
function SanityImage({ source, fallback, alt, ...props }: any) {
  const src = (source?.asset?._ref ? urlFor(source)?.url() : null) || (typeof source === 'string' ? source : fallback)
